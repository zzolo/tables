...
class Tables {
  ...
  async start() {
    ...
    try {
      ...
      this.ui._inputStream.done({
        status: 'error',
        text: this.ui._inputStream._text('Problem connecting to', e.toString())
      });
      debug(e);
      return await this.fail(e);
    }
    ...
    this.input.on('data', d => {
      ...
      this.ui._input.update({
        progress: this.inputBytes / this.input._tablesStats.size
      });
    });
    ...
    this.extractor.on('data', async d => {
      ...
      let rows = await this.loader(t, this.input, this.extractor, this.options);
  ...
  getUi() {
    ...
    let syncModelsText = (action = 'Setting up', error) =>
      `${action} tables: ${_.map(this.options.models, m =>
        chalk.cyan(m.tableName)
    ...
    let inputText = (action = 'Extracting', error) =>
      `[progress] [percent] ${action} ${chalk.magenta(
        this.options.format
      )} data ${error ? '[' + chalk.red(error) + ']' : ''}`;
...
async function main() {
  let t = new Tables({
    //db: 'mysql://root:@localhost:3306/tables',
