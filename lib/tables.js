...
class Tables {
  ...
  getUi() {
    ...
    // Input/extraction.  Format can be a function.
    let formatName = _.isString(this.options.format) ? this.options.format : 'custom';
    let inputText = (action = 'Extracting', error) =>
      (this.options.pipe ? '' : '[progress] [percent] ') +
      `${action} ${chalk.magenta(formatName)} data ${
        error ? '[' + chalk.red(error) + ']' : ''
      }`;
    ...
    return new Promise((resolve, reject) => {
      ...
      const done = (pipeExahusted = false) => {
        try {
          ...
          let model = guessModel(dataCollected, this.options, this.db);
  ...
  /**
   * Wrapper around debug to affix the ui output,
   * so that the debug can be seen
   */
  debug(v) {
    if (debug.enabled && this.ui && this.ui.output) {
      this.ui.render();
      this.ui.output.done();
      debug(v);
    }
